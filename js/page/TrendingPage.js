...
export default class TrendingPage extends Component {
  ...
  onSubscriber = (preTab, currentTab) => {
    ...
    if (
      FLAG_TAB.flag_trendingTab === currentTab &&
      changedValues.my.languageChange
    ) {
      //从设置页面切换过来
  ...
  renderMoreView() {
    let params = {
      ...
      fromPage: FLAG_TAB.flag_trendingTab,
    };
    return (
      <MoreMenu
        {...params}
...
class TrendingTab extends Component {
  ...
  onSubscriber = (preTab, currentTab) => {
    ...
    if (currentTab != FLAG_TAB.flag_trendingTab) return;
    if (
      FLAG_TAB.flag_favoriteTab === preTab &&
      changedValues.favorite.trendingChange
    ) {
      //从收藏页面切换过来,且Trending收藏有改变
  ...
  flushFavoriteState() {
    ...
    for (var i = 0, len = items.length; i < len; i++) {
      projectModels.push(
        new ProjectModel(items[i], this.checkFavorite(items[i])),
      );
    }
    this.isRender = true;
    this.updateState({
    ...
    dataRepository
      ...
      .then(wrapData => {
        ...
        if (
          ...
          wrapData &&
          wrapData.date &&
          !dataRepository.checkDate(wrapData.date)
        )
  ...
  render() {
    ...
    return <View style={GlobalStyles.listView_container}>{content}</View>;
  }
}

...
export default class SearchPage extends Component {
  ...
  saveKey() {
    ...
    if (this.checkKeyIsExist(this.keys, key)) {
      this.refs.toast.show(
        this.state.inputKey + ' already exists.',
  ...
  loadData() {
    ...
    this.cancelable = makeCancelable(
      fetch(this.genFetchUrl(this.state.inputKey)),
    );
    this.cancelable.promise
      ...
      .then(responseData => {
        if (
          ...
          this.refs.toast.show(
            this.state.inputKey + ' nothing found.',
            DURATION.LENGTH_SHORT,
          );
  ...
  flushFavoriteState() {
    ...
    for (var i = 0, len = items.length; i < len; i++) {
      projectModels.push(
        new ProjectModel(
          items[i],
          Utils.checkFavorite(items[i], this.favoritKeys),
        ),
      );
    }
    this.updateState({

...
export default class PopularPage extends Component {
  ...
  onSubscriber = (preTab, currentTab) => {
    ...
    if (FLAG_TAB.flag_popularTab === currentTab && changedValues.my.keyChange) {
      //从设置页面切换过来
...
class PopularTab extends Component {
  ...
  onSubscriber = (preTab, currentTab) => {
    ...
    if (
      ...
      changedValues.favorite.popularChange
    ) {
  ...
  flushFavoriteState() {
    ...
    for (var i = 0, len = items.length; i < len; i++) {
      projectModels.push(
        new ProjectModel(
          items[i],
          Utils.checkFavorite(items[i], this.state.favoritKeys),
        ),
      );
    }
    this.updateState({
